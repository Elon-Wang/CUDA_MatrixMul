...
int main(){
    ...
    const char Out[]="../data/MM_v5_Result.bin";
    ...
    for(int i=0;i<10;i++){
        ...
        MM_v8<<<dim3(1,128),dim3(256,1)>>>(d_A,d_B,d_C);
...
__global__ void MM_v8(float* d_A, float* d_B, float* d_C){
    int Batch = blockIdx.y;
    int tx = threadIdx.x;
    int BatchOffset = (Batch *128 *128); 

    __shared__ float M1_1[65][16];
    __shared__ float M1_2[65][16];
    __shared__ float M2_1[65][16];
    __shared__ float M2_2[65][16];
    float output[64]={0};
    int idx =tx;
    float reside;
    ...
    for (int cnt=0;cnt<8;cnt++){
        for(int chn =0;chn<16;chn++){
            if(tx<64){
                idx = tx;
                M1_1[idx][chn] = d_A[BatchOffset + cnt*16*128 + chn * 128 +idx ];
            }else if(tx<128) {
                idx = tx - 64;
                M1_2[idx][chn] = d_A[BatchOffset + cnt*16*128 + chn * 128 +idx + 64];
            }else if(tx<192) {
                idx = tx - 128;
                M2_1[idx][chn] = d_B[BatchOffset + cnt*16*128 + chn * 128 +idx ];
            }else {
                idx = tx - 192;
                M2_2[idx][chn] = d_B[BatchOffset + cnt*16*128 + chn * 128 +idx + 64];
            }
        }
        ...
        for (int chn = 0;chn<16;chn++){
            if(tx<64){
                ...
                for(int k = 0;k<64;k++){
                    // int tmp = (idx+k)%64;
                    // output[tmp] += M1_1[chn][tmp] * reside; 
                    output[k] += M1_1[k][chn] * reside; 
                } 
            }else if(tx<128) {
                idx = tx - 64;
                reside = M2_2[idx][chn];
                ...
                for(int k = 0;k<64;k++){
                    // int tmp = (idx+k)%64;
                    // output[tmp] += M1_2[chn][tmp] * reside; 
                    output[k] += M1_2[k][chn] * reside; 
                } 
            }else {
                idx = tx - 192;
                reside = M2_2[idx][chn];
    ...
    for(int k=0;k<64;k++){
        int outputOffset = BatchOffset + k*128+ tx;
        if(tx<128){
            d_C[outputOffset] = output[k];
        }else{
            d_C[outputOffset+ 63*128] = output[k];
        }
    }
}