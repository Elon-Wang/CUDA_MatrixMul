...
__global__ void MM_v1(float*,float*,float*);
...
int main(){
    ...
    for(int i=0;i<20;i++){
        ...
        MM_v1<<<dim3(128,1),dim3(32,32)>>>(d_A,d_B,d_C);
...
__global__ void MM_v1(float* d_A, float* d_B, float* d_C){
    ...
    for(int k=0;k<16;k++){
        for(int i=0;i<128;i++){
            output[k/4][k%4] += d_A[Batch*128*128+tx*4*128 + k/4*128 +i]*d_B[Batch*128*128+ty*4*128 + k%4*128 +i];
        }
