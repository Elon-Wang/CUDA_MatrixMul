...
__global__ void MM_v4(float*,float*,float*);
...
int main(){
    ...
    for(int i=0;i<20;i++){
        ...
        MM_v4<<<dim3(128,1),dim3(16,16)>>>(d_A,d_B,d_C);
...
__global__ void MM_v4(float* d_A, float* d_B, float* d_C){
    ...
    for (int cnt=0;cnt<128/16;cnt++){
        ...
        for (int j=0;j<8;j++){
            M1[ty][j*16+tx]= d_A[Batch*128*128+ j*128*16 + tx*128 + cnt*16 + ty];
        }
        for (int j=0;j<8;j++){
            M2[ty][j*16+tx]= d_B[Batch*128*128+ j*128*16 + tx*128 + cnt*16 + ty];
        }
        
        __syncthreads();
        for(int k=0;k<64;k++){
            for(int i=0;i<16;i++){
                output[k/8][k%8] += M1[i][tx*8+ k/8] * M2[i][ty*8 + k%8];
            }
        }
        __syncthreads();
    }
